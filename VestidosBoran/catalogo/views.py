...
def index(request):
    ...
    num_vestidos=Vestido.objects.all().count()
    num_solicitudes=Arriendo_vestido.objects.all().count()
    # Libros disponibles (status = 'a')
    num_solicitudes_disponibles=Arriendo_vestido.objects.filter(status__exact='a').count()
